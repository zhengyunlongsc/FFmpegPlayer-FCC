...
void VideoChannel::render() {
    ...
    double sync_threshold = 0.1;       // 同步容差：0.1 秒内不调整
    double max_frame_duration = 0.3;   // 最大帧间隔时间（超过则丢帧）
    ren_def_diff = 0;
    pts_second_diff = 0;
    ...
    while (is_playing) {
        ...
        if (ren_def_diff == 0) {
            if (audio_channel->ren_def_diff == 0) {
                releaseFrame(&frame);
                continue;
            }
            ren_def_diff = audio_channel->ren_def_diff;
            //LOGE("video-channel-cpp render tag 0-0 v_diff=%f", ren_def_diff);
        }
        ...
        if (v_a_diff < -max_frame_duration || v_clock_diff < -max_frame_duration) {
            //LOGE("video-channel-cpp drop frame v_a_diff=%f,v_clock_diff=%f size=%d", v_a_diff, v_clock_diff, frames.size());
            releaseFrame(&frame);
            continue;
        }

        if (v_clock_diff > sync_threshold) {
            //LOGE("video-channel-cpp render tag 0-2-1 sleep to sync: %f", v_clock_diff - sync_threshold);
            av_usleep(static_cast<int64_t>((v_clock_diff - sync_threshold) * 1e6));
        } else if (v_clock_diff < -sync_threshold) {
            //LOGE("video-channel-cpp render tag 0-2-2 v_clock_diff=%f", v_clock_diff);
        }
